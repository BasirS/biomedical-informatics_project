...
While this analysis was conducted independently, readers may find the work of [@gill2014rarity] provides additional insights into respiratory arrest patterns in emergency departments. Their study offers a different methodological approach and more detailed statistical analysis of similar patterns, which could be valuable for those interested in exploring this topic further.
...
```{r fig.cap="Time Series Analysis of Emergency Room Admissions (1999-2011). The plot shows monthly admission rates with seasonal patterns and the Ben Geen incident period highlighted.", fig.align='left', fig.width=10, fig.height=6, echo=FALSE}
...
To understand the underlying patterns in the hospital data, each time series was decomposed into its fundamental components. This decomposition follows the additive model:

\[Y_t = T_t + S_t + R_t\]

where each observation \(Y_t\) at time \(t\) is broken down into a trend component \(T_t\) that captures long-term changes, a seasonal component \(S_t\) that represents repeating patterns, and a remainder \(R_t\) that accounts for any unexplained variation. This decomposition helps understand why the emergency room admissions show such distinct patterns compared to critical care events - the relative sizes of these components reveal about the nature of each time series.

For instance, in emergency room admissions, it can be seen that a strong seasonal component \(S_t\) follows a clear annual pattern, while the trend \(T_t\) shows a steady increase over time. In contrast, the critical care events have a more prominent remainder component \(R_t\), suggesting these events are influenced by factors beyond regular seasonal patterns.
...
The Exponential Smoothing State Space (ETS) models that were fitted to each time series can be expressed as:

\[\hat{y}_{t+1|t} = \ell_t + b_t + s_{t-m+1}\]

This equation shows how the forecasts \(\hat{y}_{t+1|t}\) combine three key elements: the level \(\ell_t\) representing the current value, the trend \(b_t\) capturing the direction of change, and the seasonal component \(s_{t-m+1}\) accounting for periodic patterns. The model automatically determines which of these components are needed for each time series - for example, it found that critical care events don't benefit from a seasonal component, while emergency room admissions do.

This flexibility in the model structure is particularly valuable for the hospital data, where different types of events follow different patterns. The emergency room admissions, with their clear seasonal patterns, benefit from all three components, while the more irregular critical care events are better modeled using just the level and trend components.
...
To identify unusual patterns in the hospital data, multiple detection methods were employed. The first approach uses standardized scores:

\[z_t = \frac{x_t - \mu}{\sigma}\]

This z-score transformation helps identify points that deviate significantly from the expected pattern. However, since hospital events often show gradual changes rather than sudden spikes, Cumulative Sum (CUSUM) control charts were also used:

\[C_t^+ = \max(0, C_{t-1}^+ + z_t - k)\]
\[C_t^- = \min(0, C_{t-1}^- + z_t + k)\]

These equations track cumulative deviations from the expected pattern, making them particularly sensitive to the kind of gradual changes we might see in hospital events. The CUSUM approach is especially valuable for detecting the onset of unusual patterns in critical care events, where early detection could be crucial for hospital management.
...
Figures 9 through 12 present diagnostic plots for the time series models fitted to each metric. These plots offer a look at how well the models captured the underlying patterns and the characteristics of what was left over, the residuals.
...
For modeling the discrete nature of hospital events, count models were utilized. The Poisson distribution, for instance, models the probability of observing a specific number of events:

\[P(Y_t = y) = \frac{e^{-\lambda_t}\lambda_t^y}{y!}\]

This formulation is particularly appropriate for the hospital data, where counts of events (like the number of cardio-respiratory cases) are dealt with rather than continuous measurements. The parameter \(\lambda_t\) represents the expected number of events at time \(t\), which can vary over time to capture trends and seasonal patterns in hospital admissions.

The negative binomial model extends this framework to account for overdispersion, which is common in hospital data where the variance often exceeds the mean. This makes it particularly suitable for modeling critical care events, which tend to show more variability than regular admissions.

```{r fig.cap="Results of count time series models applied to critical care events are shown, with actual counts and predictions from Poisson and Negative Binomial models displayed for each type.", fig.align='left', echo=FALSE}
knitr::include_graphics("../final_visualizations/xv.count_forecast.png")
```

Figure 13 shows the results of count time series models applied to critical care events over a specific test period. The plots display the actual counts alongside the predictions from the Poisson and Negative Binomial models for each critical care type.
...
Figure 14 presents the final combined forecasts for each time series. The plots show the historical data for each metric and extend outwards to present the expected patterns and uncertainty for a future period, with prediction intervals indicating the range where future values might fall.
...
Figure 15 shows the results of the z-score analysis applied to the hospital metrics. The plots display the original time series data for each type of event and for admissions, with specific points highlighted where the analysis flagged an anomaly based on a z-score threshold.
...
Figure 16 presents the results of the Cumulative Sum (CUSUM) analysis. The plots show the original time series data for each metric, with specific points highlighted where the analysis flagged an anomaly based on the CUSUM statistic exceeding a decision interval.
...
Figure 17 displays the results of anomaly detection based on residuals. The plots show the original time series data for each metric, with specific points highlighted where the analysis flagged an anomaly using residuals from a simple 3-month moving average model.
...
Figure 18 shows the results of the early warning system applied to the hospital data. The plots display the original time series for each metric, with moments and periods flagged as unusual activity by the system highlighted in red.
...
Figure 19 presents performance metrics for the early warning system when evaluated. The bar chart shows the Sensitivity, Specificity, and F1 Score calculated for each of the different hospital metrics.
...
To assess how well these models captured the patterns in hospital data, several metrics were needed:

\[\text{MAE} = \frac{1}{n}\sum_{t=1}^n |y_t - \hat{y}_t|\]
\[\text{RMSE} = \sqrt{\frac{1}{n}\sum_{t=1}^n (y_t - \hat{y}_t)^2}\]
\[\text{MAPE} = \frac{100\%}{n}\sum_{t=1}^n \left|\frac{y_t - \hat{y}_t}{y_t}\right|\]

These equations help us understand different aspects of our models' performance. The Mean Absolute Error (MAE) gives us a straightforward measure of average prediction error, while the Root Mean Square Error (RMSE) penalizes larger errors more heavily. The Mean Absolute Percentage Error (MAPE) is particularly useful for comparing performance across different types of events, as it expresses errors as percentages of the actual values.

This comprehensive evaluation approach allows us to understand not just how accurate our predictions are, but also how the errors are distributed across different types of hospital events. For instance, we might find that while the MAE is similar across different event types, the MAPE could be higher for critical care events due to their lower baseline counts. 
